...
export default function Login() {
  const [email, setEmail] = useState("demo@auditsummar.ai");
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    login({ email, name: email.split("@")[0] || "Auditor", role: "Auditor" });
      ...
      >
        <div className="glass-card p-8 sm:p-10">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full rounded-xl bg-accent hover:bg-accent/90 text-accent-foreground font-medium h-11"
              disabled={loading}
            >
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="rounded-2xl">
        <DialogHeader>
          ...
          <DialogDescription>We'll send you a reset link.</DialogDescription>
        </DialogHeader>
        <Input placeholder="you@company.com" type="email" className="rounded-xl" />
        <DialogFooter>
          <Button onClick={() => { toast.success("Reset link sent"); setOpen(false); }} className="bg-accent hover:bg-accent/90 text-accent-foreground rounded-xl">
            Send link
          </Button>
        </DialogFooter>
      </DialogContent>
